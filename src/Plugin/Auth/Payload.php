...
final class Payload extends BasePayload {
	...
	public static function fromRequest(Request $request): static {
		...
		[$self->type, $self->handler] = match (true) {
			...
			self::hasShowUsers($request) => ['show_users', 'ShowHandler'],
			self::hasShowMyPermissions($request) => ['show_my_permissions', 'ShowHandler'],
			self::hasShowPermissions($request) => ['show_permissions', 'ShowHandler'],
	...
	public static function hasMatch(Request $request): bool {
		return (
			...
			self::hasShowUsers($request) ||
			self::hasShowMyPermissions($request) ||
			self::hasShowPermissions($request) ||
	...
	private static function hasShowUsers(Request $request): bool {
		return (str_starts_with(
				$request->error,
				'P01: syntax error, unexpected identifier, '.
				"expecting VARIABLES near 'USERS'"
			)
			&& stripos($request->payload, 'SHOW USERS') !== false);
	}
	...
	private static function hasShowPermissions(Request $request): bool {
		return (str_starts_with(
				$request->error,
				'P01: syntax error, unexpected identifier, '.
				"expecting VARIABLES near 'PERMISSIONS'"
			)
			&& stripos($request->payload, 'SHOW PERMISSIONS') !== false);
	}
	...
	private static function hasSetPassword(Request $request): bool {
		return (str_starts_with(
			$request->error,
			'P01: syntax error, unexpected identifier, '.
			"expecting VARIABLES near 'PASSWORD'"
		)
	...
	private static function parseGrantRevokeCommand(string $payload, self $self): void {
		...
		if (!preg_match($pattern, $payload, $matches)) {
			throw GenericError::create('Invalid payload: Does not match GRANT command.', true);
		}

		$command = strtolower($matches[1]);
		$action = $matches[2];
		$target = $matches[4] ?? $matches[3];
