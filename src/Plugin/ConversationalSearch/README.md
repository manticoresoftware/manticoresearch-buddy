...
The `ConversationalSearch` plugin adds SQL-managed retrieval-augmented chat to
...
The `fields` argument in `CALL CHAT` is a historical name. It
...
- Conversational Search uses the same fields to build the text context sent to the
  LLM.
...
If `CALL CHAT` does not specify a vector field, the first detected
...
The fifth `CALL CHAT` argument names a field that is not a vector
